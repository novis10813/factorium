...
class BinanceDataDownloader:
            ...
            for date in dates:
                task = asyncio.create_task(
                    self._download_single_day(
                        session, symbol, data_type, market_type, futures_type, date, download_dir, semaphore
                    )
                )
                tasks.append(task)
                    ...
                    with tempfile.TemporaryDirectory() as temp_dir:
                        ...
                        if await self._download_file(session, f"{base_url}/{filename}", data_file_path):
                            ...
                            if await self._download_file(
                                session, f"{base_url}/{checksum_filename}", checksum_file_path
                            ):
                                if await self._verify_checksum(data_file_path, checksum_file_path):
                                    ...
                                    if csv_path.exists():
                                        ...
                                        hive_path = build_hive_path(
                                            self.base_path, market, data_type, symbol, date.year, date.month, date.day
                                        )
    ...
    async def _download_file(self, session: aiohttp.ClientSession, url: str, file_path: Path) -> bool:
...
Examples:
  ...
  python -m factorium.data.downloader -s BTCUSDT -t klines -m spot -r 2024-01-01:2024-01-31
    ...
    parser.add_argument(
        "-t", "--data-type", default="trades", choices=["trades", "klines", "aggTrades", "bookTicker", "bookDepth"]
    )
