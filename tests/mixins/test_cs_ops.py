...
def test_cs_rank(sample_data):
    ...
    np.testing.assert_allclose(group1['factor'].values, expected, rtol=1e-5)
...
def test_cs_rank_strict_nan(sample_data_with_nan):
    ...
    assert ranked.data['factor'].isna().all()
...
def test_cs_zscore(sample_data):
    ...
    np.testing.assert_allclose(group1['factor'].values, expected, atol=1e-5)

def test_cs_zscore_strict_nan(sample_data_with_nan):
    f = Factor(sample_data_with_nan)
    zscored = f.cs_zscore()
    assert zscored.data['factor'].isna().all()
...
def test_cs_winsorize(sample_data):
    ...
    vals = winsorized.data['factor'].values
...
def test_cs_neutralize():
    ...
    np.testing.assert_allclose(neutralized.data['factor'].values, 0.0, atol=1e-10)
...
def test_cs_neutralize_nan(sample_data_with_nan, sample_data):
    ...
    neutralized = f_y.cs_neutralize(f_x)
    assert neutralized.data['factor'].isna().all()
