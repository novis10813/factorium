...
class TestLoadData:
    ...
    def test_triggers_download_when_files_missing(self, loader):
        ...
        with (
            patch.object(loader, "_check_all_files_exist", return_value=False),
            patch.object(loader, "_read_data_duckdb", return_value=mock_df),
            patch.object(loader.downloader, "download_data", new=MagicMock()) as mock_download,
    ...
    def test_skips_download_when_files_exist(self, loader):
        ...
        with (
            patch.object(loader, "_check_all_files_exist", return_value=True),
            patch.object(loader, "_read_data_duckdb", return_value=mock_df),
            patch("asyncio.run") as mock_asyncio,
        ):
            loader.load_data(
    ...
    def test_full_read_workflow(self, temp_data_dir):
        ...
        for i in range(3):
            ...
            hive_path = build_hive_path(
                temp_data_dir, "futures_um", "aggTrades", "BTCUSDT", date.year, date.month, date.day
            )
            hive_path.mkdir(parents=True, exist_ok=True)
